    ...
    <form class="card" novalidate @submit.prevent="onSubmit">
      ...
      <div class="field">
        ...
        <input
          ...
          :class="{ invalid: showError('documento') }"
          placeholder=" "
        />
        <small v-if="showError('documento')" class="error">{{ errors.documento }}</small>
      </div>
...
<script setup lang="ts">
  ...
  const validateField = (k: FormKey) => {
    ...
    switch (k) {
      case 'documento':
        if (!v) msg = 'El documento es obligatorio.';
        else if (!/^\d{6,12}$/.test(String(v))) msg = 'Debe tener entre 6 y 12 dígitos.';
        break;
      case 'nombre':
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      serverOk.value = false;
      serverMsg.value = 'No se pudo registrar el estudiante. Intenta nuevamente.';
    } finally {
